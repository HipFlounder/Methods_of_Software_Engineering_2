...
void Hero::sellItem(int index) {
    if (index >= 0 && index < 10 && inventory[index] != nullptr) {
        ...
        delete inventory[index]; //Lösche das Item-Objekt
        inventory[index] = nullptr; //Setze Pointer auf Null
    } else {
...
bool Hero::fight(Npc& enemy) {
    while (this->getHealth() > 0 && enemy.getHealth() > 0) {
        ...
        if (enemy.getHealth() <= 0) {
            ...
            if (loot != nullptr) {
                std::cout << "Gegenstand " << loot << " wurde zum Inventar der Heldin hinzugefuegt." << std::endl;
            } else {
...
int Hero::addEquipmentItem(Item* item) {
    ...
    throw FullGearException("addEquipmentItem(): Inventar voll!");
}
...
Item* Hero::removeEquipmentItem(int slot) {
    if(slot >= 0 && slot < 2 && gear[slot] != nullptr) {
        ...
        throw InvalidIndexException("removeEquiptmentItem(): Ungügliter Index!");
    }
}